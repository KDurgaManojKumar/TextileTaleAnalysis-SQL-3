...
SELECT details.product_name,SUM(sales.qty) AS sale_counts
FROM sales AS sales
...
WITH cte_member_revenue AS (SELECT
    member,txn_id,
    SUM(price * qty) AS revenue FROM sales
...
SELECT details.category_id,details.category_name,SUM(sales.qty) AS total_quantity,
	SUM(sales.qty * sales.price) AS total_revenue,
    SUM(sales.qty * sales.price * sales.discount)/100 AS total_discount
FROM sales AS sales
INNER JOIN product_details AS details ON sales.prod_id = details.product_id
